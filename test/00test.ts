...
describe('Order', () => {
	describe('Create', () => {
		it('should instantiate a new plain order object', () => {
			...
			assert.strictEqual(order.created instanceof Date, true);
	...
	describe('Save', () => {
		...
		it('should save and load saved order from db', async () => {
			...
			assert.strictEqual(loadedOrder.created.toDateString(), order.created.toDateString());
			assert.strictEqual(loadedOrder.created.toTimeString(), order.created.toTimeString()); // Note that milliseconds are not stored in db
